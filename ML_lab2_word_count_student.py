...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = wordsRDD.map(lambda k:(k,1)).reduceByKey(lambda k, v: k + v).count()
print uniqueWords
...
def wordCount(wordListRDD):
    ...
    """
    wordPairs = wordListRDD.map(lambda k:(k,1))
    return wordPairs.reduceByKey(lambda a, b: a + b)
...
def removePunctuation(text):
    ...
    """
    newtext = re.sub('[^a-zA-Z0-9\s]', '', text).lower().strip()
    return newtext
