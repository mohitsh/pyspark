...
def parsePoint(line):
    ...
    for i in range(len(dabba)):
        intline.append(float(dabba[i]))
    
    a = intline[0]
    b = intline[1:]
    ans = LabeledPoint(a,b)
    return ans
#‘label’ and ‘features’ 
parsedSamplePoints = parsePoint(samplePoints[0])
firstPointFeatures = parsedSamplePoints.features.toArray()
...
nTrain = len(parsedTrainData.collect())
nVal = len(parsedValData.collect())
nTest = len(parsedTestData.collect())
...
averageTrainYear = parsedTrainData.map(lambda lp: lp.label).sum()/parsedTrainData.count()
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return (labelsAndPreds.map(lambda (x,y): squaredError(x,y)).sum()/labelsAndPreds.count())**0.5
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        #predictionExample.map(lambda lp: getLabeledPrediction(weights, lp))
        errorTrain[i] = calcRMSE(labelsAndPredsTrain)

        # Calculate the `gradient`.  Make use of the `gradientSummand` function you wrote in (3a). 
        # Note that `gradient` sould be a `DenseVector` of length `d`.
        from operator import add
        gradient = trainData.map(lambda x: gradientSummand(DenseVector(w), x)).sum()
        #gradient = gradientSummand(w,labelsAndPredsTrain)
...
# TODO: Replace <FILL IN> with appropriate code
numIters = 50
#linregGradientDescent(trainData, numIters)  parsedTrainData
...
# TODO: Replace <FILL IN> with appropriate code
#labelsAndPreds = firstModel.predict(parsedValData.collect()[0].features)
...
def twoWayInteractions(lp):
    ...
    for i in lp.features:
        for j in lp.features:
            interactions.append(i*j)
        
    features = np.hstack((lp.features,interactions))
...
# TODO: Replace <FILL IN> with appropriate code
#labelsAndPredsTest = parsedTestData.map(lambda lp:(lp.label,modelInteract.predict(lp.features)))
