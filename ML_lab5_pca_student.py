...
def pca(data, k=2):
    ...
    sortedEigVals1 = np.argsort(eigVals)
    sortedEigVals  = sortedEigVals1[::-1]
    indi = sortedEigVals[:k]
    sortedEigVecs1 = np.argsort(eigVecs)
    sortedEigVecs = sortedEigVecs1[::-1]
    #reqComponent = eigVecs[:,(1,k-1)]
    #reqComponent = eigVecs[:,:k]
    reqComponent = eigVecs[:,indi]
    # Return the `k` principal components, `k` scores, and all eigenvalues
    datascores = data.map(lambda x: x.dot(reqComponent))
    return (reqComponent,datascores,eigVals[::-1])
...
# TODO: Replace <FILL IN> with appropriate code
mn = rawData.flatMap(lambda x: min(parse(x)[1])).min()
mx = rawData.flatMap(lambda x: max(parse(x)[1])).max()
